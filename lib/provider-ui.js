...
function useProviderQuotaCache(providerKey, providerName, quota, auth) {
	...
	const cached = useMemo(() => answered && signedOut ? void 0 : headerQuotaFromCache(peekCachedUsage(providerKey)), [
		answered,
		signedOut,
		providerKey
	]);
...
function ProviderMark(props) {
	...
	switch (raw === "cursor-agent" || raw === "cursor" || raw === "acp-cursor" ? "llm-cursor" : raw === "claude-agent" || raw === "claude" || raw === "claude-code" || raw === "acp-claude" ? "llm-claude" : raw.startsWith("llm-") ? raw : raw === "opencode" ? "llm-opencode-go" : "llm-" + raw) {
		case "llm-claude": return jsx(Svg, {
			viewBox: "0 0 24 24",
			children: jsx("path", {
				fill: "currentColor",
...
//#endregion
export { ProviderCardHeader, ProviderMark, ProviderQuotaMeter, ProviderRoleBadge, normalizeQuotaRemaining, providerQuotaHeaderProps, providerUiCss, useProviderQuotaCache };
