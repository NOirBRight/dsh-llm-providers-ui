...
`;
		...
		function pluginAction(section, matcher) {
			return [...section.querySelectorAll("button")].find((node) => node instanceof HTMLElement && node.closest("[data-provider-model]") === null && node.getAttribute("data-c-own") === null && matcher.test((node.textContent ?? "").replace(/\s+/gu, " ").trim()));
		}
		function markChrome(node, kind) {
			if (node !== null && node !== void 0 && node.getAttribute("data-c-plugin-chrome") !== kind) node.setAttribute("data-c-plugin-chrome", kind);
		}
		...
		* provider shows the same copy, icons and geometry. */
		function paintModelsChrome(section) {
			...
			if (sectionToggle !== void 0 && sectionToggle.dataset.cOpened !== "1") {
			...
			const addButton = [...root.querySelectorAll("button")].find((node) => node instanceof HTMLElement && node.getAttribute("data-c-own") === null && node.closest("[data-provider-model]") === null && ADD_TEXT.test((node.textContent ?? "").replace(/\s+/gu, " ").trim()));
			...
			if (!(pluginHead instanceof HTMLElement)) {
				const found = [...section.children].find((node) => node instanceof HTMLElement && node.tagName === "DIV" && node.querySelector("[data-provider-model]") === null && node.querySelectorAll("button").length > 0);
				pluginHead = found instanceof HTMLElement ? found : null;
			}
			markChrome(pluginHead instanceof HTMLElement ? pluginHead : void 0, "header");
			const zh = isZh() || /[\u4e00-\u9fff]/u.test(pluginHead instanceof HTMLElement ? pluginHead.textContent ?? "" : "");
			for (const child of section.children) if (child instanceof HTMLElement && child.tagName === "P" && child.getAttribute("data-c-models-hint") === null) markChrome(child, "hint");
			...
			const done = sortButton !== void 0 && /done|完成/iu.test((sortButton.textContent ?? "").trim());
			setOwnButton(head.sort, done ? MODEL_ICONS.check : MODEL_ICONS.sort, done ? zh ? "完成排序" : "Done sorting" : zh ? "排序" : "Sort", sortButton === void 0 || sortButton.hasAttribute("disabled"));
			setOwnButton(head.catalog, MODEL_ICONS.plus, zh ? "从账户目录选取" : "Choose from account", catalogButton === void 0 || catalogButton.hasAttribute("disabled"));
			...
			if (!(addOwn instanceof HTMLButtonElement)) {
				...
				addOwn.addEventListener("click", () => {
					pluginAction(section, ADD_TEXT)?.click();
				});
		...
		function ensureModelsHead(section, zh) {
			...
			if (found instanceof HTMLElement) {
				...
				if (count instanceof HTMLElement && expand instanceof HTMLButtonElement && sort instanceof HTMLButtonElement && catalog instanceof HTMLButtonElement) return {
					head: found,
					count,
					expand,
					sort,
			...
			sort.addEventListener("click", () => {
				pluginAction(section, SORT_TEXT)?.click();
			});
			catalog.addEventListener("click", () => {
				pluginAction(section, CATALOG_TEXT)?.click();
			});
		...
		function paintAccount(section, t, linked) {
			...
			const email = (/Signed in as\s+(.+)/iu.exec(raw) ?? /以\s*(.+?)\s*身份登录/u.exec(raw) ?? (/@/.test(raw) ? [raw, raw] : null))?.[1]?.trim();
			if (paragraph === null || email === void 0 || email.length === 0) return;
		...
		function bindProvidersSection(listRegisteredKeys, subscribe, readPage, onReorder, roleOf, onShowSidebarUsage, headerOf, readUsage, subscribeUsage, accountOf, onRefresh) {
		...
		*/
		function ProvidersSection(props) {
			...
			const [modelCounts, setModelCounts] = (0, react.useState)({});
			...
			(0, react.useLayoutEffect)(() => {
				...
				const timers = [
					...
					600,
					1200
				].map((ms) => window.setTimeout(paint, ms));
				const later = () => {
			...
			(0, react.useLayoutEffect)(() => {
				...
				document.querySelectorAll("[data-model-probe]").forEach((node) => {
					if (!(node instanceof HTMLElement) || node.dataset.modelProbe === void 0) return;
					const count = countModels(node);
					if (count !== void 0) next[node.dataset.modelProbe] = count;
				});
				const full = document.querySelector("[data-providers-section] .c-full");
				if (full instanceof HTMLElement && detail !== void 0) {
					const count = countModels(full);
					if (count !== void 0) next[detail] = count;
				}
				setModelCounts((prev) => {
					let changed = false;
					const merged = { ...prev };
					for (const [key, count] of Object.entries(next)) if (merged[key] !== count) {
						merged[key] = count;
						changed = true;
					}
				...
				children: [
					...
					detail === void 0 ? (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [
						...
						children: [
							...
							(0, react_jsx_runtime.jsx)("span", { children: props.usageSummaries?.find((entry) => entry.providerKey === detail)?.name ?? detail })
						]
					}),
					body
				]
