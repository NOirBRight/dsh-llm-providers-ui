	...
	factory: (require) => {
		...
		*/
		...
		const copy = {
			zh: {
				...
				usageLoggedOut: "未登录",
				usageUnsupported: "不支持查询",
				usageStale: "额度已过期",
				usageError: "加载失败",
				usageExpired: "已过期",
				usageShowProvider: "在侧栏显示 {name}",
				usageDetails: "{name} 额度详情",
				usageBack: "返回全部 Provider",
				usageRefreshProvider: "刷新 {name}",
				usageNoProviders: "暂无可查询的 Provider",
				usageNoneVisible: "没有显示的 Provider",
				usageFilterHint: "使用筛选按钮选择要在侧栏显示的 Provider",
				usageOpenFilter: "打开筛选",
				...
				usageNoMatch: "没有匹配的 Provider",
				usageReorder: "调整顺序: {name}",
				nav: "LLM 供应商",
				title: "LLM 供应商",
			...
			en: {
				...
				usageUnsupported: "Quota unavailable",
				usageStale: "Quota expired",
				usageError: "Could not load quota",
				usageExpired: "Expired",
				usageShowProvider: "Show {name} in sidebar",
				usageDetails: "{name} quota details",
				usageBack: "Back to all providers",
				usageRefreshProvider: "Refresh {name}",
				usageNoProviders: "No queryable providers",
				usageNoneVisible: "No visible providers",
				usageFilterHint: "Use the filter to choose which providers appear in the sidebar",
				usageOpenFilter: "Open filter",
				usageChooseProviders: "Choose sidebar providers",
				usageRefreshAll: "Refresh all",
				usageVisibility: "Sidebar visibility",
				usageVisibilityHint: "Only affects Provider Usage, not the model list",
				usageCloseFilter: "Close filter",
				usageSearch: "Search providers",
				usageShowAll: "Show all {n}",
				usageNoMatch: "No matching providers",
				usageReorder: "Reorder: {name}",
				nav: "LLM Providers",
				title: "LLM Providers",
		...
		*/
		...
		function createProviderUsageStore(rpc, readerForKey) {
			...
			let configuredKeys = [];
			let unconnectedKeys = /* @__PURE__ */ new Set();
			...
			const startRead = (key, refresh) => {
				...
				reader.read(rpc, refresh, controller.signal).then((result) => {
					...
					const next = result.status === "ready" ? {
						...
						windows: orderUsageWindows(result.windows)
					} : {
						providerKey: key,
						name: reader.name,
			...
			const canRead = (key) => configuredKeys.includes(key) && !snapshot.hiddenKeys.includes(key) && !unconnectedKeys.has(key);
			const setEmptyUsage = (key) => {
				const reader = readerForKey(key);
				if (reader !== void 0) current.set(key, {
					providerKey: key,
					name: reader.name,
					status: unconnectedKeys.has(key) ? "logged-out" : "loading",
					windows: []
				});
			};
			const visibleKeys = (keys) => {
				return (keys ?? configuredKeys).filter(canRead);
			};
			...
			return {
				...
				configure: (config) => {
					...
					for (const [key, controller] of active) if (!canRead(key)) {
						controller.abort();
						active.delete(key);
					}
					for (let index = queued.length - 1; index >= 0; index -= 1) {
						const item = queued[index];
						if (item !== void 0 && !canRead(item.key)) queued.splice(index, 1);
					}
					...
					for (const key of configuredKeys) {
						...
						else if (existing === void 0) setEmptyUsage(key);
					}
					sync(false);
					startPoll();
				},
				...
				invalidate: (keys) => {
					...
					for (const key of targets) setEmptyUsage(key);
					publish();
					sync(true, keys);
				},
				dispose: () => {
...
`;
		...
		*/
		function catalogAllOpen(items, expanded) {
			if (items === void 0 || items.length === 0) return false;
			const open = new Set(expanded ?? []);
			return items.every((row) => open.has(row.rowId));
		}
		function toggleAllModelRows(models) {
			const items = models.items ?? [];
			const expanded = models.expanded ?? [];
			const allOpen = catalogAllOpen(items, expanded);
			if (models.onToggle !== void 0) {
				for (const row of items) if (allOpen === expanded.includes(row.rowId)) models.onToggle(row.rowId);
				return;
			}
			models.onToggleAll?.();
		}
						...
						children: [
							(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [(0, react_jsx_runtime.jsxs)("div", {
									...
									children: [
										props.models.onToggleAll === void 0 && props.models.onToggle === void 0 ? null : (0, react_jsx_runtime.jsxs)("button", {
											type: "button",
											className: "c-btn quiet c-icon-label",
											"aria-pressed": catalogAllOpen(props.models.items, props.models.expanded),
											onClick: () => {
												const models = props.models;
												if (models === void 0) return;
												toggleAllModelRows(models);
											},
											children: [(0, react_jsx_runtime.jsx)(DetailIcon, { path: ICON.expand }), catalogAllOpen(props.models.items, props.models.expanded) ? props.copy.collapseAll : props.copy.expandAll]
										}),
							...
							(0, react_jsx_runtime.jsx)("div", {
								...
								children: props.models.items === void 0 ? null : (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [(0, react_jsx_runtime.jsx)(SortableList, {
									...
									renderItem: (row, index) => {
										...
										const expanded = props.models?.sorting !== true && props.models?.expanded?.includes(row.rowId) === true;
										return (0, react_jsx_runtime.jsxs)("div", {
...
`;
		...
		function ProvidersSection(props) {
			...
			const renderCard = (item) => {
				...
				const node = props.renderSlot?.(PROVIDERS_ITEM_SLOT, {
					...
					...account === void 0 || account.state === "unknown" ? {} : { accountState: account.state },
				...
				const identity = (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [(0, react_jsx_runtime.jsx)("span", {
						...
						children: [(0, react_jsx_runtime.jsx)("span", {
							className: "c-name",
							children: props.nameOf?.(item.key) ?? summary?.name ?? item.key
						}), (0, react_jsx_runtime.jsx)(ProviderRoleBadge, { ...role === "llm" ? {} : { role } })]
					}), (0, react_jsx_runtime.jsxs)("div", {
						className: "c-sub",
						children: [
							(0, react_jsx_runtime.jsx)("span", { className: "c-dot" + (linked(link) ? " good" : "") }),
							t(link),
		...
		*/
		...
		function ProviderRow(props) {
			...
			const primary = summary.status === "ready" || summary.status === "stale" ? pickPrimaryWindow(summary.windows) ?? summary.windows[0] : void 0;
			const headline = primary === void 0 ? summary.status === "loading" ? "…" : "—" : windowValueText(primary);
			const label = summary.name + " " + (primary === void 0 ? props.t(STATUS_KEY[summary.status]) : headline) + (summary.status === "stale" ? " · " + props.t("usageExpired") : "");
			...
			return (0, react_jsx_runtime.jsx)("div", {
				...
				children: (0, react_jsx_runtime.jsxs)("button", {
					...
					children: [(0, react_jsx_runtime.jsxs)("span", {
						className: "pu-mark",
						children: [(0, react_jsx_runtime.jsx)(ProviderMark, { providerKey: summary.providerKey }), summary.status === "stale" && (0, react_jsx_runtime.jsx)("span", {
							className: "pu-stale",
							"aria-hidden": "true",
							children: "*"
						})]
		...
		function UsageDetail(props) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				className: "pu-detail",
				"aria-label": props.t("usageDetails").replace("{name}", summary.name),
				children: [(0, react_jsx_runtime.jsxs)("div", {
		...
		function ProviderUsagePanel(props) {
			...
			const matches = normalizedQuery === "" ? providers : providers.filter((summary) => summary.name.toLowerCase().includes(normalizedQuery));
			const allVisible = providers.length > 0 && visible.length === providers.length;
			let body;
			if (visible.length === 0) body = (0, react_jsx_runtime.jsxs)("div", {
				className: "pu-empty",
				children: [
					(0, react_jsx_runtime.jsx)("div", { children: providers.length === 0 ? props.t("usageNoProviders") : props.t("usageNoneVisible") }),
					(0, react_jsx_runtime.jsx)("div", { children: props.t("usageFilterHint") }),
					(0, react_jsx_runtime.jsx)("button", {
						type: "button",
						...
						children: props.t("usageOpenFilter")
					})
				]
			});
			...
			return (0, react_jsx_runtime.jsxs)("section", {
				...
				"aria-label": props.t("usageTitle"),
				children: [
					(0, react_jsx_runtime.jsx)("style", { children: providerUiCss + panelCss }),
					detail === void 0 && (0, react_jsx_runtime.jsxs)("div", {
						className: "pu-head",
						children: [(0, react_jsx_runtime.jsx)("span", {
							className: "pu-title",
							children: props.t("usageTitle")
						}), (0, react_jsx_runtime.jsxs)("span", {
						...
						children: [
							(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [(0, react_jsx_runtime.jsxs)("div", { children: [(0, react_jsx_runtime.jsx)("div", {
									...
									children: props.t("usageVisibility")
								}), (0, react_jsx_runtime.jsx)("div", {
									className: "pu-popover-sub",
									children: props.t("usageVisibilityHint")
								})] }), (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: "pu-icon-btn pu-popover-close",
									"aria-label": props.t("usageCloseFilter"),
									onClick: () => {
										setFilterOpen(false);
									},
									children: "×"
							...
							(0, react_jsx_runtime.jsxs)("div", {
								...
								children: [(0, react_jsx_runtime.jsx)("button", {
									...
									children: props.t("usageShowAll").replace("{n}", String(providers.length))
								}), matches.length === 0 ? (0, react_jsx_runtime.jsx)("p", {
									className: "pu-no-match",
									children: props.t("usageNoMatch")
		...
		function ProviderUsageAction(props) {
			...
			const showSidebarUsage = (0, react.useSyncExternalStore)(props.subscribeForm, props.readShowSidebarUsage, props.readShowSidebarUsage);
		...
		function installProviderUsage(ctx, orderForm, directory, onStore) {
			let connection;
			try {
			...
			const writeList = (field, value) => {
				...
				orderForm.set(field, [...value]).then((accepted) => {
		...
		var ProviderDirectory = class {
			...
			*/
			register(declaration) {
				...
				if (declaration.binding !== void 0 && (declaration.binding.channel === "" || declaration.binding.endpoint === "")) throw new Error("Provider binding channel and endpoint must not be empty");
				if (declaration.catalogId !== void 0) {
					for (const [key, entry] of this.entries) if (key !== declaration.key && entry.catalogId === declaration.catalogId) throw new Error("Provider catalogId is already registered: " + declaration.catalogId);
				}
				const entry = {
					...
					...declaration.catalogId === void 0 ? {} : { catalogId: declaration.catalogId },
					...declaration.binding === void 0 ? {} : { binding: {
						channel: declaration.binding.channel,
						endpoint: declaration.binding.endpoint
					} },
			...
			*/
			...
			catalogRoutes() {
				const routes = {};
				for (const [key, entry] of this.entries) if (entry.catalogId !== void 0) routes[entry.catalogId] = key;
				return routes;
			}
			/**
			...
			*/
			nativeBindings() {
				...
				for (const entry of this.entries.values()) {
					if (entry.role !== "agent" || entry.catalogId === void 0 || entry.binding === void 0) continue;
					bindings.push({
						provider: entry.catalogId,
						channel: entry.binding.channel,
						endpoint: entry.binding.endpoint
					});
				}
				return bindings;
			}
		...
		*/
		function installMissingOwnerDiagnostic(orderForm) {
			let warned = false;
			const check = () => {
		...
		*/
		function installMissingSectionDiagnostic(ctx, orderForm) {
		...
		*/
		function installProvidersPageTransaction(ctx, orderForm, t, directory, usageRef) {
			let stopSection;
			let stopNav;
			const unmount = () => {
			...
			const mount = () => {
				if (stopSection !== void 0 || !pageVisible(orderForm.getSnapshot())) return;
				stopSection = ctx.slots.inject("settings.section", () => ctx.slots.register({
					...
					orderForm.set("order", keys).then((accepted) => {
						if (!accepted) console.warn("[dsh-llm-providers-ui] Host refused provider order update");
					}).catch((error) => {
						console.warn("[dsh-llm-providers-ui] failed to save provider order", error);
					});
				}, (key) => directory.roleOf(key), (show) => {
					orderForm.set("showSidebarUsage", show).then((accepted) => {
						if (!accepted) console.warn("[dsh-llm-providers-ui] Host refused sidebar usage visibility update");
					}).catch((error) => {
						console.warn("[dsh-llm-providers-ui] failed to save sidebar usage visibility", error);
					});
		...
		function apply(ctx, _config = {}) {
			ctx.effect(() => {
				...
				try {
					...
					const orderForm = ctx.configForms.get(PROVIDERS_CONFIG_ID);
					...
					const usageRef = {};
					try {
						disposers.push(installProviderUsage(ctx, orderForm, directory, (store) => {
