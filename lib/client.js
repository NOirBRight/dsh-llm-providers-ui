window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		new Map(Object.entries(PROVIDER_ROUTES).map(([key, route]) => [route, key]));
		...
		function decodeProviderOrder(value) {
			...
			return {
				order: Array.isArray(rawOrder) ? rawOrder.filter((entry) => typeof entry === "string" && entry.length > 0) : [],
				hiddenUsageProviders: Array.isArray(rawHidden) ? rawHidden.filter((entry) => typeof entry === "string" && entry.length > 0) : []
			};
		}
		/**
		...
		*/
		...
		function pickPrimaryWindow(windows) {
			...
			for (const window of windows) {
				...
				if (best === void 0 || remaining < (best.remainingPercent ?? Number.POSITIVE_INFINITY)) best = window;
			}
		...
		function primaryValueText(window) {
			...
			return window.remainingPercent === void 0 ? window.valueText : String(window.remainingPercent) + "%";
		}
		/** Only low/warn headlines take red/amber; everything else stays neutral. */
		function usageTone(remainingPercent) {
			if (remainingPercent === void 0) return "neutral";
		...
		function windowTooltip(windows) {
			return windows.map((window) => window.label + " " + (window.remainingPercent === void 0 ? window.valueText : String(window.remainingPercent) + "%") + (window.resetsAt === void 0 ? "" : " · 重置 " + window.resetsAt)).join("；");
		}
		...
		function ProviderRow(props) {
			...
			const tone = primary === void 0 ? "neutral" : usageTone(primary.remainingPercent);
			const headline = primary === void 0 ? STATUS_TEXT[summary.status] ?? "" : primaryValueText(primary);
			const shown = summary.windows.slice(0, 3);
			return (0, react_jsx_runtime.jsxs)("button", {
				type: "button",
				className: "pu-row" + (tone === "low" ? " pu-low" : tone === "warn" ? " pu-warn" : "") + (props.active ? " pu-active" : ""),
					...
					children: [
						(0, react_jsx_runtime.jsxs)("span", {
							...
							children: [(0, react_jsx_runtime.jsx)("span", {
								className: "pu-icon",
								"aria-hidden": true,
								children: summary.name.trim().charAt(0)
						...
						summary.status === "stale" ? (0, react_jsx_runtime.jsx)("span", {
							className: "pu-stale",
							title: "上次更新 " + (summary.fetchedAt ?? "未知"),
							children: "已过期"
						}) : null
					]
				}), shown.length > 0 ? (0, react_jsx_runtime.jsx)("span", {
					className: "pu-windows",
					style: { gridTemplateColumns: "repeat(" + String(shown.length) + ", minmax(0, 1fr))" },
					children: shown.map((window) => (0, react_jsx_runtime.jsxs)("span", {
						className: "pu-window",
						title: window.label + (window.resetsAt === void 0 ? "" : " · 重置 " + window.resetsAt),
						children: [(0, react_jsx_runtime.jsx)("small", { children: window.shortLabel }), (0, react_jsx_runtime.jsx)("b", { children: window.remainingPercent === void 0 ? window.valueText : String(window.remainingPercent) + "%" })]
					}, window.id))
				}) : null]
			});
		}
		...
		function ProviderUsagePanel(props) {
			...
			let body;
			if (props.unavailable === true) body = (0, react_jsx_runtime.jsx)("p", {
				className: "pu-empty",
				children: "Provider 用量暂不可用，请稍后刷新重试。"
			});
			else if (props.loading === true && visible.length === 0) body = (0, react_jsx_runtime.jsx)("p", {
				className: "pu-empty",
				children: "正在加载 Provider 用量…"
			});
			else if (visible.length === 0) body = (0, react_jsx_runtime.jsxs)("div", {
				className: "pu-empty",
				children: [
								...
								children: [
									(0, react_jsx_runtime.jsxs)("label", {
										...
										children: [(0, react_jsx_runtime.jsx)("input", {
											...
											checked: allVisible,
											onChange: (event) => {
												if (event.target.checked) props.onShowAll();
												else props.onHideAll();
											}
										}), (0, react_jsx_runtime.jsx)("span", {
		...
		function decodePercentUsage(usage) {
			...
			for (const value of usage.windows) {
				...
				const remaining = item.unit === "percent" ? percentage(100 - item.used) : item.limit === 0 ? void 0 : percentage(100 * (1 - item.used / item.limit));
				const resetAt = item.resetsAt ?? viewReset;
				windows.push({
					id: item.id,
					label,
					shortLabel: shortLabel(label),
					...remaining === void 0 ? { valueText: displayNumber(Math.max(0, item.limit - item.used)) + " / " + displayNumber(item.limit) } : {
		...
		function decodeCommandCodeUsage(usage) {
			...
			for (const [key, label] of [["fiveHour", "5-hour"], ["weekly", "Weekly"]]) {
				...
				const remaining = item.cap === 0 ? void 0 : percentage(100 * (1 - item.used / item.cap));
				windows.push({
					id: key,
					label,
					shortLabel: shortLabel(label),
					...remaining === void 0 ? { valueText: displayNumber(Math.max(0, item.cap - item.used)) + " / " + displayNumber(item.cap) } : {
						remainingPercent: remaining,
						valueText: percentageText(remaining)
					},
		...
		const readerByKey = new Map([
			{
				...
				name: "Cursor",
				read: (rpc, refresh, signal) => rpc.call("/cursor", "usage/read", refresh ? { refresh: true } : {}, signal).then((result) => result.ok ? usageResult(result.value, decodePercentUsage) : {
					status: "error",
			...
			},
			{
				...
				name: "Grok",
				read: (rpc, refresh, signal) => rpc.call("/grok", "usage/read", refresh ? { refresh: true } : {}, signal).then((result) => result.ok ? usageResult(result.value, decodePercentUsage) : {
					status: "error",
					message: result.error.message
				})
			},
			{
				...
				name: "Ollama Cloud",
				read: (rpc, _refresh, signal) => rpc.call("/ollama-cloud", "usage/read", {}, signal).then((result) => result.ok ? usageResult(result.value, (value) => decodeFractionUsage(["session", "weekly"], value)) : {
					status: "error",
					message: result.error.message
				})
			},
			{
				...
				name: "CommandCode",
				read: (rpc, _refresh, signal) => rpc.call("/commandcode", "usage/read", {}, signal).then((result) => result.ok ? usageResult(result.value, decodeCommandCodeUsage) : {
					status: "error",
					message: result.error.message
				})
			},
			{
				...
				name: "OpenCode Go",
				read: (rpc, _refresh, signal) => rpc.call("/opencode-go", "usage/read", {}, signal).then((result) => result.ok ? usageResult(result.value, (value) => decodeFractionUsage([
					"session",
					"weekly",
					"monthly"
				], value)) : {
		...
		function providerUsageReader(key) {
		...
		function createProviderUsageStore(rpc) {
			...
			const read = (key, refresh) => {
				...
				if (reader === void 0 || rpc === void 0 || active.has(key) || disposed) return;
			...
			return {
				...
				configure: (registeredKeys, savedOrder, hiddenKeys) => {
					const ordered = applySavedOrder(registeredKeys, savedOrder).filter((key) => providerUsageReader(key) !== void 0);
		...
		function currentProviderKey(state) {
			if (state.current === void 0) return void 0;
		...
		function ProviderUsageAction(props) {
			...
			const activeProviderKey = currentProviderKey(props.useSessions((state) => state));
		...
		function installProviderUsage(ctx, orderScope) {
			...
			try {
				...
				if (candidate !== void 0 && candidate !== null && typeof candidate === "object" && "rpc" in candidate) rpc = candidate.rpc;
			} catch {}
			...
			const hideAll = () => {
				writeHidden(usage.getSnapshot().providers.map((provider) => provider.providerKey));
			};
			reconcile();
