		...
		*/
		...
		function createProviderUsageStore(rpc, readerForKey) {
			...
			const visibleKeys = (keys) => {
				return (keys === void 0 ? configuredKeys : keys.filter((key) => configuredKeys.includes(key))).filter((key) => !snapshot.hiddenKeys.includes(key) && !unconnectedKeys.has(key));
			};
			...
			return {
				...
				configure: (config) => {
					...
					for (const [key, controller] of active) if (!configuredKeys.includes(key) || snapshot.hiddenKeys.includes(key) || unconnectedKeys.has(key)) {
						controller.abort();
						active.delete(key);
					}
					for (let index = queued.length - 1; index >= 0; index -= 1) {
						const item = queued[index];
						if (item !== void 0 && (!configuredKeys.includes(item.key) || snapshot.hiddenKeys.includes(item.key) || unconnectedKeys.has(item.key))) queued.splice(index, 1);
					}
