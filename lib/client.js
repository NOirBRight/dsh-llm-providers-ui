...
`;
		...
		*/
		function ProvidersSection(props) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				style: pageStyle,
				children: [
					(0, react_jsx_runtime.jsx)("style", { children: providerUiCss + providerShellCss }),
					(0, react_jsx_runtime.jsx)("header", { children: (0, react_jsx_runtime.jsx)("h2", {
						style: titleStyle,
						children: t("title")
					}) }),
		...
		* its own boot settles, so absence at apply time is not evidence of a missing shell.
		*/
		const MISSING_SECTION_GRACE_MS = 15e3;
		/**
		* Warn once when an available Host namespace has no Web section declaration.
		* The check is deferred by {@link MISSING_SECTION_GRACE_MS}, and a declaration
		* arriving in that window cancels it for the diagnostic's lifetime. A shell that
		* declares the slot and later collapses it is a reload, not a missing shell.
		...
		*/
		function installMissingSectionDiagnostic(ctx, orderScope) {
			...
			let declared = false;
			const check = () => {
				if (warned || declared || !pageVisible(orderScope.getSnapshot()) || ctx.slots.spec("settings.section") !== void 0) return;
				warned = true;
			...
			const stopSection = ctx.slots.subscribe("settings.section", () => {
				if (ctx.slots.spec("settings.section") === void 0) return;
				declared = true;
				clearTimeout(timer);
			});
