					...
					renderedItems.map((item, index) => {
						...
						return (0, react_jsx_runtime.jsxs)("div", {
							...
							onPointerDown: (event) => {
								...
								if (!(event.clientX - row.getBoundingClientRect().left <= 44 || event.target instanceof Element && event.target.closest("[data-sortable-handle]") !== null) && event.target instanceof Element && event.target.closest("a, input, select, textarea, label, button") !== null) return;
								startDrag(event, id);
							},
