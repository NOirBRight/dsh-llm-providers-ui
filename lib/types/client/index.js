...
function installSectionTransaction(ctx, orderScope, t, directory, usageRef) {
    ...
    const mount = () => {
        ...
        }, keys => { void orderScope.set('order', keys); }, key => directory.roleOf(key), show => { void orderScope.set('showSidebarUsage', show); }, key => directory.headerOf(key), () => usageRef.current?.getSnapshot().providers ?? [], listener => usageRef.current?.subscribe(listener) ?? (() => undefined), key => directory.accountOf(key), key => { key === undefined ? usageRef.current?.refresh() : usageRef.current?.refresh([key]); })));
        stopSection = section;
        try {
