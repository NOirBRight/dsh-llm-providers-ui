...
export function createProviderUsageStore(rpc, readerForKey) {
    ...
    const publish = () => {
        ...
        if (signedOut.length > 0)
            dropPersistedUsageKeys(signedOut);
        snapshot = {
    ...
    const setEmptyUsage = (key) => {
        const reader = readerForKey(key);
        if (reader !== undefined)
            current.set(key, { providerKey: key, name: reader.name, status: unconnectedKeys.has(key) ? 'logged-out' : 'loading', windows: [] });
    };
    ...
    return {
        ...
        configure: config => {
            ...
            for (let index = queued.length - 1; index >= 0; index -= 1) {
                ...
                if (item !== undefined && (!canRead(item.key)))
            ...
            for (const key of configuredKeys) {
                ...
                if (existing?.status === 'logged-out' || hasUsageData(existing))
                    continue;
