    ...
    return {
        ...
        configure: config => {
            ...
            for (const [key, controller] of active)
                if (!configuredKeys.includes(key) || snapshot.hiddenKeys.includes(key) || unconnectedKeys.has(key)) {
                    controller.abort();
                    active.delete(key);
                }
            for (let index = queued.length - 1; index >= 0; index -= 1) {
                const item = queued[index];
                if (item !== undefined && (!configuredKeys.includes(item.key) || snapshot.hiddenKeys.includes(item.key) || unconnectedKeys.has(item.key)))
            ...
            for (const key of configuredKeys) {
                if (unconnectedKeys.has(key)) {
                    const reader = readerForKey(key);
                    if (reader !== undefined)
                        current.set(key, { providerKey: key, name: reader.name, status: 'logged-out', windows: [] });
                    continue;
                }
