...
const plainItemStyle = { minWidth: 0, display: 'flex', flexDirection: 'column', padding: '4px 0' };
 ...
 */
export function SortableList({ items, getId, renderItem, dragLabel, onReorder, disabled = false, chrome = 'row', sorting = true, moveButtons = false, moveUpLabel, moveDownLabel, }) {
    ...
    const startDrag = (event, id) => {
        if (!interactive || dragGhostRef.current !== null)
            return;
    ...
    return (_jsxs("div", { "data-sortable-card": card ? '' : undefined, "data-sortable-plain": plain ? '' : undefined, style: { ...listStyle, ...(card ? { gap: 12 } : {}), ...(plain ? { gap: 0 } : {}) }, children: [card ? _jsx("style", { children: cardCss }) : null, plain || moveButtons ? _jsx("style", { children: touchCss }) : null, renderedItems.map((item, index) => {
                const id = getId(item);
                ...
                return (_jsxs("div", { ref: (node) => { setRowRef(id, node); }, "data-sortable-row": "true", style: {
                        ...
                        if (target instanceof Element && target.closest('a, input, select, textarea, label, button:not([data-sortable-handle])') !== null)
                            return;
                        startDrag(event, id);
                    }, children: [_jsx("button", { type: "button", "data-sortable-handle": "", style: { ...(bare ? bareHandleStyle : handleStyle), display: showHandle ? 'flex' : 'none', ...(plain ? { borderRight: 0 } : {}), cursor: disabled ? 'default' : draggedId === null ? 'grab' : 'grabbing' }, "aria-label": dragLabel(item, index), "aria-grabbed": dragging, title: dragLabel(item, index), disabled: disabled, hidden: !showHandle, onDragStart: (event) => { event.preventDefault(); }, onPointerDown: (event) => { startDrag(event, id); }, onKeyDown: (event) => { handleKeyDown(event, id); }, children: _jsx(IconGrip, {}) }), _jsx("div", { "data-sortable-item": "", style: rowItemStyle, children: renderItem(item, index) }), moveButtons
                            ? (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", "data-sortable-move": "up", style: { ...moveButtonStyle, display: showHandle ? 'inline-flex' : 'none' }, "aria-label": upLabel(item, index), title: upLabel(item, index), disabled: !interactive || index === 0, hidden: !showHandle, onClick: () => { moveBy(id, -1); }, children: "\u2191" }), _jsx("button", { type: "button", "data-sortable-move": "down", style: { ...moveButtonStyle, display: showHandle ? 'inline-flex' : 'none' }, "aria-label": downLabel(item, index), title: downLabel(item, index), disabled: !interactive || index === renderedItems.length - 1, hidden: !showHandle, onClick: () => { moveBy(id, 1); }, children: "\u2193" })] }))
                            : null] }, id));
            }), dragGhost !== null && draggedItem !== undefined
                ? (_jsxs("div", { "data-sortable-row": "true", "data-sortable-ghost": "true", "aria-hidden": "true", ref: setGhostInert, style: {
                        ...rowChromeStyle,
                        gridTemplateColumns: rowGridColumns,
                        position: 'fixed',
                    ...
                    }, children: [_jsx("div", { "data-sortable-handle": "", style: { ...handleStyle, display: showHandle ? 'flex' : 'none', ...(plain ? { borderRight: 0 } : {}), cursor: 'grabbing' }, children: _jsx(IconGrip, {}) }), _jsx("div", { "data-sortable-item": "", style: rowItemStyle, children: renderItem(draggedItem, renderedItems.findIndex(item => getId(item) === draggedId)) }), moveButtons && showHandle
                            ? (_jsxs(_Fragment, { children: [_jsx("span", { "aria-hidden": "true", style: { ...moveButtonStyle, visibility: 'hidden' }, children: "\u2191" }), _jsx("span", { "aria-hidden": "true", style: { ...moveButtonStyle, visibility: 'hidden' }, children: "\u2193" })] }))
                            : null] }))
                : null] }));
}
