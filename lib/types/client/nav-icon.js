...
function sharedRegistry() {
    ...
    if (existing !== undefined)
        return existing;
    const created = { nextOwner: 0, states: new WeakMap() };
...
function flattenFailure(failures, error) {
    if (error instanceof AggregateError && error.errors.length > 0) {
        for (const nested of error.errors)
            flattenFailure(failures, nested);
    }
    else {
        failures.push(error);
    }
...
function snapshotsEqual(left, right) {
    if (left.innerHTML !== right.innerHTML || left.attributes.length !== right.attributes.length)
        return false;
...
function restoreSvg(svg, original) {
    ...
    try {
        for (let index = 0; index < svg.attributes.length; index += 1) {
            const attribute = svg.attributes.item(index);
            if (attribute !== null)
                current.push({ name: attribute.name, namespace: attribute.namespaceURI, value: attribute.value });
        }
    }
    catch (error) {
        failures.push(error);
    ...
    for (const attribute of current) {
        try {
            if (attribute.namespace === null)
                ...
                svg.removeAttributeNS(attribute.namespace, attribute.name);
        }
        catch (error) {
            failures.push(error);
        }
    ...
    for (const attribute of original.attributes) {
        try {
            if (attribute.namespace === null)
                ...
                svg.setAttributeNS(attribute.namespace, attribute.name, attribute.value);
        }
        catch (error) {
            failures.push(error);
        }
...
function latestInstallRecord(installation, svg) {
    ...
    for (const record of installation.records) {
        if (record.svg === svg && record.active)
            latest = record;
    }
    return latest;
}
...
function pruneState(registry, svg, state) {
    ...
    if (state.records.length === 0)
        registry.states.delete(svg);
}
function rootRecord(record) {
    let root = record;
    while (root.previous !== undefined)
        root = root.previous;
    return root;
}
...
function patchSvg(registry, installation, button, svg, force) {
    ...
    if (!force && top !== undefined && top.marker !== installation.marker)
        return;
    if (own !== undefined && own === top) {
        if (snapshotsEqual(current, own.owned))
            return;
 ...
 */
export function installProvidersNavIcon() {
    ...
    const schedule = () => {
        if (installation.disposed || installation.scheduled || installation.requestFrame === undefined)
            return;
        installation.scheduled = true;
        try {
            const handle = installation.requestFrame(flush);
            if (installation.scheduled)
                installation.frame = handle;
        }
        catch (error) {
            installation.scheduled = false;
            installation.frame = undefined;
