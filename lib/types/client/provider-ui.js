 ...
 */
export function useProviderQuotaCache(providerKey, providerName, quota, auth) {
    ...
    useEffect(() => {
        ...
        if (signedOut) {
            if (answered)
                dropPersistedUsageKeys([providerKey]);
            return;
        }
        ...
        if (withheld === true)
            return;
        if (quota !== null)
            rememberHeadlineQuota(providerKey, providerName, quota);
    ...
    const cached = useMemo(() => (answered && signedOut ? undefined : headerQuotaFromCache(peekCachedUsage(providerKey))), [answered, signedOut, providerKey]);
 ...
 */
export function providerQuotaHeaderProps(quota, options) {
    if (quota !== null)
        return { quota };
