...
import { PROVIDERS_LOCALE_NS, PROVIDERS_ITEM_SLOT } from '../order.js';
...
function ProviderUsageAction(props) {
    ...
    if (!props.wide || !showSidebarUsage)
        return null;
    return (_jsx(ProviderUsagePanel, { t: props.t, providers: usage.providers, hiddenKeys: usage.hiddenKeys, refreshing: usage.refreshing, onRefresh: key => { key === undefined ? props.usage.refresh() : props.usage.refresh([key]); }, onToggleVisibility: props.toggleVisibility, onShowAll: props.showAll, onReorder: keys => { props.reorder(keys); } }));
}
...
export function installProviderUsage(ctx, orderForm, directory, onStore) {
    let connection;
    try {
    ...
    const writeList = (field, value) => {
        ...
        void orderForm.set(field, [...value]).then(accepted => {
            if (!accepted)
                console.warn('[dsh-llm-providers-ui] Host refused Provider Usage ' + field + ' update');
        }).catch(error => {
