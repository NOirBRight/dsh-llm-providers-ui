...
function ProviderUsageAction(props) {
    ...
    if (!props.wide)
        return null;
    return (_jsx(ProviderUsagePanel, { providers: usage.providers, hiddenKeys: usage.hiddenKeys, ...activeProviderKey === undefined ? {} : { currentProviderKey: activeProviderKey }, refreshing: usage.refreshing, onRefresh: key => { key === undefined ? props.usage.refresh() : props.usage.refresh([key]); }, onToggleVisibility: props.toggleVisibility, onShowAll: props.showAll, onReorder: keys => { props.reorder(keys); } }));
}
