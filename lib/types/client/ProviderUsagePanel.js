...
function ProviderRow(props) {
    ...
    return (_jsxs("div", { className: "pu-cell", children: [_jsxs("div", { role: "button", tabIndex: 0, className: 'pu-row' + (tone === undefined ? '' : ' pu-' + tone) + (props.active ? ' pu-active' : ''), "aria-label": summary.name + ' ' + (primary === undefined ? STATUS_TEXT[summary.status] : headline), onClick: props.onSelect, onKeyDown: event => { if (event.key === 'Enter' || event.key === ' ') {
                    event.preventDefault();
                    props.onSelect();
                } }, children: [_jsx("span", { className: "pu-mark", children: _jsx(ProviderMark, { providerKey: summary.providerKey, fallback: providerInitial(summary.name) }) }), _jsxs("span", { className: "pu-copy", children: [_jsxs("span", { className: "pu-name", children: [summary.name, summary.status === 'stale' ? ' · 已过期' : ''] }), _jsx("b", { className: 'pu-primary' + (primary === undefined ? ' pu-empty-text' : ''), children: headline })] })] }), _jsx("button", { type: "button", className: 'pu-icon-btn pu-row-refresh' + (summary.refreshing === true ? ' pu-spinning' : ''), "aria-label": '刷新 ' + summary.name, onClick: props.onRefresh, children: summary.refreshing === true ? _jsx("span", { className: "pu-mini-spin" }) : _jsx(RefreshIcon, {}) })] }));
}
...
export function ProviderUsagePanel(props) {
    ...
    if (visible.length === 0) {
        ...
        body = (_jsx("div", { className: "pu-rows", children: visible.map(summary => (_jsx(ProviderRow, { summary: summary, active: summary.providerKey === props.currentProviderKey, onSelect: () => { setFilterOpen(false); setDetailKey(summary.providerKey); }, onRefresh: () => { props.onRefresh(summary.providerKey); } }, summary.providerKey))) }));
    }
