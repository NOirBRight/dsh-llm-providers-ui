...
function usageToneClass(remainingPercent) {
...
function ProviderRow(props) {
    ...
    return (_jsxs("button", { type: "button", className: 'pu-row' + toneClass + (props.active ? ' pu-active' : ''), "aria-label": summary.name + ' ' + headline, title: summary.windows.length > 0 ? windowTooltip(summary.windows) : headline, children: [_jsxs("span", { className: "pu-top", children: [_jsxs("span", { className: "pu-id", children: [_jsx("span", { className: "pu-icon", "aria-hidden": true, children: providerInitial(summary.name) }), _jsx("span", { className: "pu-name", children: summary.name })] }), _jsx("b", { className: "pu-primary", children: headline }), summary.status === 'stale'
                        ? _jsx("span", { className: "pu-stale", title: '上次更新 ' + (summary.fetchedAt ?? '未知'), children: "\u5DF2\u8FC7\u671F" })
                        : null] }), shown.length > 0
                ? (_jsx("span", { className: "pu-windows", style: { gridTemplateColumns: 'repeat(' + String(shown.length) + ', minmax(0, 1fr))' }, children: shown.map(quotaWindow => (_jsxs("span", { className: "pu-window", title: windowTooltip([quotaWindow]), children: [_jsx("small", { children: quotaWindow.shortLabel }), _jsx("b", { children: windowValueText(quotaWindow) })] }, quotaWindow.id))) }))
                : null] }));
}
