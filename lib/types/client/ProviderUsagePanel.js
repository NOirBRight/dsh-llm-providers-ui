...
import { pickPrimaryWindow } from './usage.js';
...
function ProviderRow(props) {
    ...
    return (_jsxs("div", { className: "pu-cell", children: [_jsxs("div", { role: "button", tabIndex: 0, className: 'pu-row' + (tone === undefined ? '' : ' pu-' + tone), "aria-label": summary.name + ' ' + (primary === undefined ? STATUS_TEXT[summary.status] : headline), onClick: props.onSelect, onKeyDown: event => { if (event.key === 'Enter' || event.key === ' ') {
                    event.preventDefault();
                    props.onSelect();
                } }, children: [_jsx("span", { className: "pu-mark", children: _jsx(ProviderMark, { providerKey: summary.providerKey }) }), _jsxs("span", { className: "pu-copy", children: [_jsxs("span", { className: "pu-name", children: [summary.name, summary.status === 'stale' ? ' · 已过期' : ''] }), _jsx("span", { className: 'pu-primary' + (primary === undefined ? ' pu-empty-text' : ''), children: headline })] })] }), _jsx("button", { type: "button", className: 'pu-icon-btn pu-row-refresh' + (summary.refreshing === true ? ' pu-spinning' : ''), "aria-label": '刷新 ' + summary.name, onClick: props.onRefresh, children: summary.refreshing === true ? _jsx("span", { className: "pu-mini-spin" }) : _jsx(RefreshIcon, {}) })] }));
}
function UsageDetail(props) {
    const summary = props.summary;
    return (_jsxs("div", { className: "pu-detail", "aria-label": summary.name + ' 额度详情', children: [_jsxs("div", { className: "pu-detail-head", children: [_jsx("button", { type: "button", className: "pu-icon-btn", "aria-label": "\u8FD4\u56DE\u5168\u90E8 Provider", onClick: props.onBack, children: _jsx("svg", { viewBox: "0 0 20 20", "aria-hidden": "true", children: _jsx("path", { d: "M12.5 4.5 7 10l5.5 5.5" }) }) }), _jsx("span", { className: "pu-mark", children: _jsx(ProviderMark, { providerKey: summary.providerKey }) }), _jsx("span", { className: "pu-detail-name", children: summary.name }), _jsx("button", { type: "button", className: 'pu-icon-btn' + (summary.refreshing === true ? ' pu-spinning' : ''), "aria-label": '刷新 ' + summary.name, onClick: props.onRefresh, children: summary.refreshing === true ? _jsx("span", { className: "pu-mini-spin" }) : _jsx(RefreshIcon, {}) })] }), _jsx("div", { className: "pu-detail-sub", children: "\u5269\u4F59\u989D\u5EA6" }), summary.windows.length === 0
                ...
                : summary.windows.map(quotaWindow => {
                    ...
                    return (_jsxs("div", { className: "pu-win", children: [_jsxs("div", { className: "pu-win-top", children: [_jsx("span", { className: "pu-tip-label", children: quotaWindow.label }), _jsx("span", { className: 'pu-tip-value' + (tone === undefined ? '' : ' pu-' + tone), children: windowValueText(quotaWindow) })] }), remaining === undefined ? null : _jsx("progress", { className: 'pu-bar' + (tone === undefined ? '' : ' pu-' + tone), max: 100, value: remaining }), reset === undefined ? null : _jsxs("div", { className: "pu-tip-reset", children: ["\u91CD\u7F6E ", reset] })] }, quotaWindow.id));
                })] }));
}
...
export function ProviderUsagePanel(props) {
    ...
    if (visible.length === 0) {
        ...
        body = (_jsx("div", { className: "pu-rows", children: visible.map(summary => (_jsx(ProviderRow, { summary: summary, onSelect: () => { setFilterOpen(false); setDetailKey(summary.providerKey); }, onRefresh: () => { props.onRefresh(summary.providerKey); } }, summary.providerKey))) }));
    }
    return (_jsxs("section", { "data-provider-usage-panel": true, "aria-label": "Provider Usage", children: [_jsx("style", { children: panelCss }), _jsxs("div", { className: "pu-head", children: [_jsx("span", { className: "pu-title", children: "Provider Usage" }), _jsxs("span", { className: "pu-actions", children: [_jsx("button", { type: "button", className: "pu-icon-btn", "aria-label": "\u9009\u62E9\u4FA7\u680F\u663E\u793A\u7684 Provider", "aria-expanded": filterOpen, onClick: () => { setFilterOpen(open => !open); }, children: _jsxs("svg", { viewBox: "0 0 20 20", "aria-hidden": "true", children: [_jsx("path", { d: "M3 5h8M15 5h2M9 10h8M3 10h2M3 15h6M13 15h4" }), _jsx("circle", { cx: "13", cy: "5", r: "2" }), _jsx("circle", { cx: "7", cy: "10", r: "2" }), _jsx("circle", { cx: "11", cy: "15", r: "2" })] }) }), _jsx("button", { type: "button", className: 'pu-icon-btn' + (props.refreshing === true ? ' pu-spinning' : ''), "aria-label": "\u5237\u65B0\u7528\u91CF", onClick: () => { props.onRefresh(); }, children: _jsxs("svg", { viewBox: "0 0 20 20", "aria-hidden": "true", children: [_jsx("path", { d: "M16.2 7A6.5 6.5 0 1 0 16 13.5" }), _jsx("path", { d: "M16.2 3.8V7H13" })] }) })] })] }), _jsx("div", { className: 'pu-stage' + (detail === undefined ? '' : ' pu-stage-open'), children: detail === undefined ? body : _jsx(UsageDetail, { summary: detail, onBack: () => { setDetailKey(undefined); }, onRefresh: () => { props.onRefresh(detail.providerKey); } }) }), filterOpen
                ? (_jsxs("section", { className: "pu-popover", role: "dialog", "aria-label": "\u4FA7\u680F\u663E\u793A", onKeyDown: event => { if (event.key === 'Escape') {
                        setFilterOpen(false);
                        setDetailKey(undefined);
                    } }, children: [_jsxs("div", { className: "pu-popover-head", children: [_jsxs("div", { children: [_jsx("div", { className: "pu-popover-title", children: "\u4FA7\u680F\u663E\u793A" }), _jsx("div", { className: "pu-popover-sub", children: "\u53EA\u5F71\u54CD Provider Usage\uFF0C\u4E0D\u5F71\u54CD\u6A21\u578B\u5217\u8868" })] }), _jsx("button", { type: "button", className: "pu-icon-btn pu-popover-close", "aria-label": "\u5173\u95ED\u7B5B\u9009", onClick: () => { setFilterOpen(false); }, children: "\u00D7" })] }), _jsx("input", { ref: searchRef, className: "pu-search", type: "search", "aria-label": "\u641C\u7D22 Provider", placeholder: "\u641C\u7D22 Provider", value: query, onChange: event => { setQuery(event.target.value); } }), _jsxs("div", { className: "pu-filter-list", children: [_jsx("button", { type: "button", className: "pu-filter-item pu-filter-all", disabled: allVisible, onClick: props.onShowAll, children: '显示全部 ' + String(props.providers.length) + ' 个' }), matches.length === 0 ? _jsx("p", { className: "pu-no-match", children: "\u6CA1\u6709\u5339\u914D\u7684 Provider" }) : query.trim() === '' && props.onReorder !== undefined && matches.length > 1
                                    ? (_jsx(SortableList, { items: [...matches], getId: summary => summary.providerKey, dragLabel: summary => '调整顺序: ' + summary.name, onReorder: next => { props.onReorder?.(next.map(summary => summary.providerKey)); }, renderItem: summary => (_jsx(FilterRow, { summary: summary, hidden: hidden.has(summary.providerKey), onToggle: visible => { props.onToggleVisibility(summary.providerKey, visible); } })) }))
                                    : matches.map(summary => (_jsx(FilterRow, { summary: summary, hidden: hidden.has(summary.providerKey), onToggle: visible => { props.onToggleVisibility(summary.providerKey, visible); } }, summary.providerKey)))] })] }))
                : null] }));
}
