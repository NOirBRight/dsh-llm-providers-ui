...
function usageTone(remainingPercent) {
    if (remainingPercent !== undefined && remainingPercent <= 15)
        return 'low';
    if (remainingPercent !== undefined && remainingPercent <= 35)
        return 'warn';
    return undefined;
}
...
function ProviderRow(props) {
    ...
    return (_jsxs("div", { role: "group", className: 'pu-row' + (tone === undefined ? '' : ' pu-' + tone) + (props.active ? ' pu-active' : ''), "aria-label": summary.name + ' ' + headline, children: [_jsxs("span", { className: "pu-top", children: [_jsx("span", { className: "pu-icon", "aria-hidden": true, children: providerInitial(summary.name) }), _jsx("span", { className: "pu-name", children: summary.name }), summary.status === 'stale'
                        ? _jsx("span", { className: "pu-stale", title: staleUpdated === undefined ? undefined : '上次更新 ' + staleUpdated, children: "\u5DF2\u8FC7\u671F" })
                        : null] }), primary === undefined
                ? _jsx("span", { className: "pu-status", children: headline })
                : (_jsxs(_Fragment, { children: [_jsxs("span", { className: "pu-metric", title: quotaTooltip(primary), children: [_jsx("small", { className: "pu-primary-label", children: primary.shortLabel }), _jsx("b", { className: "pu-primary", children: headline })] }), primary.remainingPercent === undefined
                            ? null
                            : (_jsx("span", { className: "pu-meter", "aria-hidden": true, children: _jsx("span", { className: "pu-meter-fill", style: { width: String(Math.max(0, Math.min(100, primary.remainingPercent))) + '%' } }) }))] })), details.length > 0
                ? (_jsx("span", { className: "pu-windows", children: details.map(quotaWindow => {
                        const detailTone = usageTone(quotaWindow.remainingPercent);
                        return (_jsxs("span", { className: 'pu-window' + (detailTone === undefined ? '' : ' pu-window-' + detailTone), title: quotaTooltip(quotaWindow), children: [_jsx("small", { children: quotaWindow.shortLabel }), _jsx("b", { children: windowValueText(quotaWindow) })] }, quotaWindow.id));
                    }) }))
                : null] }));
}
