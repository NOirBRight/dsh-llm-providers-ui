...
export const OrderConfig = z.object({
    ...
    showSidebarUsage: z.boolean().default(true),
});
...
export function apply(ctx, _config = {}) {
    if (!allowDshRuntime(ctx.logger, 'dsh-llm-providers-ui', ['@deepseek-ai/dsh-settings']))
        return;
    const install = (settings) => {
        settings.installSection(ctx, PROVIDERS_SETTINGS_NS, OrderConfig, { order: [], hiddenUsageProviders: [], usageOrder: [], showSidebarUsage: true }, {
            setSource: () => undefined,
            onChange: () => undefined,
        });
    };
