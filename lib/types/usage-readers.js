...
const PERIOD_RANK = { M: MONTHLY_PERIOD_RANK, W: 5, D: 4, CURS: 3, S: 1, A: 0, L: 0, CR: -1 };
...
export function pickPrimaryWindow(windows) {
    ...
    for (const quotaWindow of windows) {
        if (quotaWindow.remainingPercent === undefined)
            continue;
        if (best === undefined || periodRank(quotaWindow) > periodRank(best))
            best = quotaWindow;
    }
    ...
    if (best !== undefined && best.remainingPercent === 100 && !nonEmptyString(best.resetsAt) && periodRank(best) < MONTHLY_PERIOD_RANK) {
        ...
        for (const quotaWindow of windows) {
            if (quotaWindow === best || !nonEmptyString(quotaWindow.resetsAt) || quotaWindow.remainingPercent === undefined)
                continue;
            if (fallback === undefined || periodRank(quotaWindow) > periodRank(fallback))
...
function displayWindowRank(quotaWindow) {
    ...
    if (id === 'fivehour' || id === 'five-hour' || id === '5h' || shortLabelValue === '5h')
        return 0;
    if (id === 'weekly' || id === 'week' || shortLabelValue === 'w')
        return 1;
    if (id === 'monthly' || id === 'month' || shortLabelValue === 'm')
        return 2;
    return 3;
}
export function orderUsageWindows(windows) {
