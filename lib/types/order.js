...
export function decodeProviderOrder(value) {
    if (value === null || typeof value !== 'object' || Array.isArray(value))
        return { order: [], hiddenUsageProviders: [], usageOrder: [], showSidebarUsage: true };
    const record = value;
    return {
...
export function providerKeyForRoute(route) {
 ...
 */
export function sortCatalogGroups(groups, saved = [], catalogKeys = {}) {
    if (saved.length === 0)
        return [...groups];
    const idByKey = new Map();
    for (const [id, key] of Object.entries(catalogKeys))
        idByKey.set(key, id);
    const ranked = applySavedOrder(groups.map(group => ownCatalogKey(catalogKeys, group.id)).filter((key) => key !== undefined), saved);
