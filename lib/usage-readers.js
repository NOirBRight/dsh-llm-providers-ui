...
async function readCodexUsage(rpc, signal) {
	...
	while (!signal.aborted) {
		...
		last = result.ok ? decodeCodexAuthStatus(result.value) : credentialFailure(result.error) ? { status: "logged-out" } : {
			status: "error",
...
function clearProviderUsageCache() {
	memoryUsageCache = /* @__PURE__ */ new Map();
	storageRemove();
}
...
//#endregion
export { clearProviderUsageCache, createCodexUsageReader, createCommandCodeUsageReader, createCursorUsageReader, createGrokUsageReader, createOllamaUsageReader, createOpenCodeGoUsageReader, dropPersistedUsageKeys, formatResetInstant, formatResetLabel, hasUsageData, headerQuotaFromCache, nonEmptyString, nonNegativeNumber, orderUsageWindows, peekCachedUsage, pickPrimaryWindow, readUsageCache, recordUsageValue, rememberCachedUsage, rememberHeadlineQuota, writeUsageCache };
