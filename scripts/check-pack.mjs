...
const CONSUMER_SMOKE = [
  ...
  "const owner = {}",
  "const settings = { configure(...args) { configuredPolicies.push(args); return () => {} } }",
  "host.apply({ logger: { warn() {} }, fiber: owner, get(name) { return name === 'settings' ? settings : undefined }, effect(apply) { return apply() }, inject() { throw new Error('Settings injection unexpectedly required') } })",
  "if (configuredPolicies.length !== 1 || configuredPolicies[0]?.[0]?.auto !== false || configuredPolicies[0]?.[1] !== owner) throw new Error('installed Host presentation policy invariants failed')",
