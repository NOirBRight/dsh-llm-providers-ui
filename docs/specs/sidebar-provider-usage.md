...
## 当前设计（v0.2.11，替代下方历史原型的布局与登录状态规则）

- 网页和移动端固定每行四个无边框「图标＋主额度」按钮，超过四个换行；不加折叠按钮。
- 点击原地进入详情，隐藏外层标题栏；返回、图标、名称、刷新合成紧凑单行，返回概览后恢复筛选和刷新。
- 桌面行高 32px，触屏按钮保留 44px 触控区域；详情内容收紧间距。
- 已知未配置／未登录的 Provider 不显示；优先读取 Provider directory 的账号状态，并兼容 usage reader 的 logged-out。自动隐藏不写入手动隐藏列表，登录后恢复。
- 暂时失败、未知账号状态及 0% 额度不视为未登录。过期值在概览用星号标记，并在 accessible name 和详情明确注明。
- 无百分比时保留供应商文本额度；加载中为 …，无数据为 —。悬停提示只是补充，所有状态均可通过点击或键盘查看。
- 保留已发布的额度窗口顺序、缓存及模型目录路由，不回退既有版本行为。

以下保留历史原型规格，冲突处以上述当前设计为准。
...
2. 周期最长窗口的剩余整数百分比；空数据为 —。点卡片进入详情页看全部窗口、进度条和本地重置时间。右上角可单票刷新（精细指针下悬停显示，触屏始终显示）。
...
- 设置页与侧边栏共用同一份 last-good 快照缓存，规则见 §6。
...
- 刷新失败保留上次可用值并标为过期；只有已知退出登录、重新认证要求、凭据失效（`INVALID_CREDENTIAL`）、登录成功或账户切换，以及写入新凭据时才删除该条，避免上个账户的额度残留。
- 卸载时中止未完成请求。
