# Task Panel 额度详情 · 已锁定 B

## 用户确认的锁定基线

| 设计 | 原型代码基线 | 原型分支 |
|---|---|---|
| Provider 设置 C（含全局侧栏显示开关） | `93d085e` | `prototype/provider-settings-system-v2` |
| Task Panel B（含全局刷新、自适应高度、精简信息） | `904226e` | `prototype/task-panel-usage-popover` |

两份设计均已获用户确认锁定，后续实施以这些版本和对应规范为准；不再自主调整布局或交互。新增设计变更需重新确认，锁定不等于授权修改生产代码。
