...
- `dsh-llm-providers-ui/client` (Web): owner plugin wiring plus `ProviderDirectory`, `ProviderDeclaration`, `ProviderRole`, `ProviderUsageReader`, and vendor `create*UsageReader` factories. Built artifact: `lib/client.js` (ModuleLoader CJS) + `lib/types/client`. Do not import `./src/*`.
...
Provider plugins register their card under `settings.provider.item` with their `settingsNs` key and, from `dsh-llm-providers-ui/client`, register `{ key, role, usage }` on `ctx.providerDirectory` inside an effect. The returned disposer owns the registration. Unregistered cards keep the LLM badge and do not receive a Provider Usage tile.
